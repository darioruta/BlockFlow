...
class TestPUSH():
    def __init__(self, configs, block_port_translator, numa, block_device_translator, blocks_config):
        ...
        self.block_port_translator = block_port_translator
        ...
        self.clients = []
        self.numa_node = numa
        self.blocks_configs =  blocks_config
        
        self.block_device_translator = block_device_translator

        script_dir = os.path.dirname(os.path.abspath(__file__)) 
        script_dir = script_dir + "/multi_gpu_results/"+ self.family_test + f"/results_conf_{self.blocks_configs}" 
        ...
        if self.with_dispatcher:
            ...
            self.dispatcher_port = block_port_translator[self.dispatcher_name]
        ...
        if self.with_collector:
            ...
            self.collector_port = block_port_translator[self.collector_name]
        ...
        for c_config in self.clients_configs:
            ...
            f_pass = [(b_name, self.block_port_translator[b_name]) for b_name in f_pass]
    ...
    def deploy_blocks(self, f_pass):
        for name, port in f_pass:
            if name not in [x["name"] for x in self.blocks_active]:
                ...
                device =  self.block_device_translator[name]
                p = mp.Process(target=self.block_call, args=(name, port, self.with_tcp, device, self.zero_copy, self.raw_data_dir, self.block_device_translator))
                p.start()
    ...
    @staticmethod
    def block_call(name, port, with_tcp, device, zero_copy, path_to_store, block_device_translator):
        block = Block_dynamic_connections(name, port, with_tcp, device, zero_copy, path_to_store, block_device_translator)
        block.run()
        return
...
def doTest1(translator, numa):
    ...
    ###### BLOCK CONFIG A - ALL BLOCKS SAME DEVICE ######
    with open("/home/druta/workspace/test/test_multi_GPU/base_config/block_device_translator_A.json", "r") as f:
        block_device_translator = json.load(f)
    f.close()

    for t_conf in configs:
        print(f"\n\n #### STARTING NEW TEST #### \n\n {t_conf["test_name"]} - BLOCK CONF: A")
        t = TestPUSH(t_conf, translator, numa, block_device_translator, blocks_config="A")
        t.run_test()
        time.sleep(5)
    

    ###### BLOCK CONFIG C - 2 BLOCKS ON THE SAME DEVICE ######
    with open("/home/druta/workspace/test/test_multi_GPU/base_config/block_device_translator_C.json", "r") as f:
        block_device_translator = json.load(f)
    f.close()

    for t_conf in configs:
        print(f"\n\n #### STARTING NEW TEST #### \n\n {t_conf["test_name"]} - BLOCK CONF: C")
        t = TestPUSH(t_conf, translator, numa, block_device_translator, blocks_config="C")
        t.run_test()
        time.sleep(5)


    ###### BLOCK CONFIG D - ALL BLOCKS DIFFERENT DEVICE ######
    with open("/home/druta/workspace/test/test_multi_GPU/base_config/block_device_translator_D.json", "r") as f:
        block_device_translator = json.load(f)
    f.close()

    for t_conf in configs:
        print(f"\n\n #### STARTING NEW TEST #### \n\n {t_conf["test_name"]} - BLOCK CONF: D")
        t = TestPUSH(t_conf, translator, numa, block_device_translator, blocks_config="D")
        t.run_test()
        time.sleep(5)
...
def doTest4(translator, zero_copy, numa):
    ...
    def doTest(configs, translator, numa, blocks_dev_translator, blocks_conf):
        for t_conf in configs:
            print(f"\n\n #### STARTING NEW TEST #### \n\n {t_conf["test_name"]}")
            t = TestPUSH(t_conf, translator, numa, blocks_dev_translator, blocks_conf)
            t.run_test()
            time.sleep(15)
        return
    ...
    if zero_copy:
        ...
        doTest(configs, translator, numa, block_device_translator, blocks_conf="A")
        ...
        doTest(configs, translator, numa, block_device_translator, blocks_conf="C")
        ...
        doTest(configs, translator, numa, block_device_translator, blocks_conf="D")
...
def main(path):
    ...
    with open("/home/druta/workspace/test/test_final_pipeline/base_config/block_port_translator.json", "r") as f:
        block_port_translator = json.load(f)
    f.close()

    for t_conf in configs:
        print(f"\n\n #### STARTING NEW TEST #### \n\n {t_conf["test_name"]}")
        t = TestPUSH(t_conf, block_port_translator)
        t.run_test()
        time.sleep(5)
...
if __name__=="__main__":
    ...
    #numa = sys.argv[1] or 0
    numa = 0
    print(f"Running numa node: {numa}")

    ###### TEST 1 ######
    doTest1(block_port_translator, numa)

    ###### TEST 3 ######
    #doTest3(block_port_translator, numa)
    
    ###### TEST 4 ######
    #doTest4(block_port_translator, zero_copy=False, numa=numa)
    
    ###### TEST 4 - ZERO COPY  ######
    #doTest4(block_port_translator, zero_copy=True, numa=numa)
    
    ###### TEST 5 ######
    #doTest5(block_port_translator, zero_copy=False, numa=numa)

    ###### TEST 5 ZERO COPY ######
    #doTest5(block_port_translator, zero_copy=True, numa=numa)
